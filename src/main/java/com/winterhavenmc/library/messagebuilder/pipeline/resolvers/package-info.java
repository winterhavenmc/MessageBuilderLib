/**
 * Provides a hierarchy of {@link Resolver} implementations responsible for converting
 * macro-related input values into string representations for use in templated messages.
 * <p>
 * The {@code resolvers} package contains the core components of the macro resolution
 * pipeline in the {@code MessageBuilderLib} library. Each {@link Resolver} translates
 * values stored in a MacroObjectMap into one or more entries in a {@code MacroStringMap},
 * keyed by {@code MacroKey} placeholders.
 *
 * <h2>Design Overview</h2>
 * <ul>
 *   <li>{@link AtomicResolver} handles atomic or primitive values (e.g. {@code String}, {@code Number},
 *       {@code Duration}) and maps them directly to the base macro key.</li>
 *   <li>{@link CompositeResolver} handles structured or compound values using a registry of
 *       Adapter instances, extracting multiple sub-values mapped to dot-notated subkeys.</li>
 *   <li>{@link FieldResolver} serves as a delegating resolver that applies multiple resolvers in order,
