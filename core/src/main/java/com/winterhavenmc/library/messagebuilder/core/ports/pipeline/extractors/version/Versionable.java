...
package com.winterhavenmc.library.messagebuilder.core.ports.pipeline.extractors.version;
...
import static com.winterhavenmc.library.messagebuilder.core.ports.pipeline.extractors.Adapter.BuiltIn.VERSION;
	...
	default MacroStringMap extractVersion(final ValidMacroKey baseKey, final AdapterCtx ctx)
	{
		return baseKey.append(VERSION).isValid()
				.map(macroKey -> new MacroStringMap()
						.with(macroKey, formatVersion(this.getVersion()).orElse(UNKNOWN_VALUE)))
				.orElseGet(MacroStringMap::empty);
	}


	Predicate<String> VALID_VERSION = version -> version != null && !version.isBlank();


	static Optional<String> formatVersion(final String versionString)
	{
		return (VALID_VERSION.test(versionString))
